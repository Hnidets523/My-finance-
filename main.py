...
HF_API_KEY = os.getenv("HF_API_KEY")  # <<< додано для AI-помічника
if not BOT_TOKEN:
...
(
    ...
    AI_CHAT       # <<< додано стан для AI-помічника
) = range(10)
...
def main_menu_ikb():
    return ikb([
        ...
        [("🤖 AI-помічник", "ai:start")]  # <<< додано кнопку AI
    ])
...
async def ask_ai(prompt: str) -> str:
    """Запит до HuggingFace API (універсальна модель з інструкціями)."""
    if not HF_API_KEY:
        return "⚠️ AI недоступний: не налаштовано HF_API_KEY у Railway."
    url = "https://api-inference.huggingface.co/models/meta-llama/Llama-3.2-3B-Instruct"
    headers = {"Authorization": f"Bearer {HF_API_KEY}"}
    payload = {
        "inputs": f"You are a helpful assistant with a focus on personal finance when relevant.\n\nUser: {prompt}\nAssistant:",
        "parameters": {"max_new_tokens": 220, "temperature": 0.7, "top_p": 0.95}
    }
    try:
        ...
        # інколи HF повертає {"error": "..."} — підхопимо
        if isinstance(res, dict) and "error" in res:
            return f"⚠️ Помилка AI: {res['error']}"
        return "⚠️ Вибач, не знайшов відповідь."
    except Exception as e:
        return f"⚠️ Помилка підключення до AI."

# Стан AI: вступ та обробка
async def ai_intro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 *AI-помічник*\n\n"
        "Можу допомогти з фінансами, інвестиціями та будь-якими питаннями.\n"
        "Просто напиши запитання — відповім максимально корисно 👇",
        parse_mode="Markdown",
        reply_markup=ikb([[("🏠 Головне меню", "main:open")]])
    )
    return AI_CHAT
...
async def ai_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # обмежимо відповідь по довжині (на випадок дуже довгих генерацій)
    if len(reply) > 3500:
        reply = reply[:3500] + "\n\n…(скорочено)"
    await update.message.reply_text(f"🤖 {reply}", reply_markup=ikb([[("🏠 Головне меню", "main:open")]]))
    return AI_CHAT  # залишаємо в режимі чату; користувач може задавати наступні питання
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # -------- AI кнопка --------
    if data == "ai:start":
        # переведемо користувача у режим AI чату через повідомлення
        await q.message.reply_text(
            "🤖 AI-помічник активний. Напиши своє питання 👇",
            reply_markup=ikb([[("🏠 Головне меню", "main:open")]])
        )
        return AI_CHAT
    ...
    if data == "quiz:start":
        ...
        context.user_data["quiz_mistakes"] = []  # (qtxt, chosen_letter, correct_letter, correct_text)
    ...
    if data.startswith("quiz:ans:"):
        ...
        # якщо натисли стару кнопку — ігноруємо
        if qidx != pos:
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            MAIN: [CallbackQueryHandler(on_cb),
                   MessageHandler(filters.TEXT & ~filters.COMMAND, ai_response)],  # даємо можливість писати AI й з меню
