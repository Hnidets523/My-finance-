...
(
    ...
    AI_CHAT          # режим чату з помічником
) = range(10)
...
INTRO_TEXT = (
    ...
    "• 🧠 Відповідати на запитання через AI-помічника\n\n"
...
def hf_generate(user_text: str) -> str:
    if not HF_API_KEY:
        return "⚠️ Помічник недоступний: не задано HF_API_KEY."
    url = f"https://api-inference.huggingface.co/models/{HF_MODEL}"
    headers = {"Authorization": f"Bearer {HF_API_KEY}"}
    # простий інструктивний промпт (універсальний)
    prompt = (
        "You are a helpful Ukrainian assistant. Answer clearly and concisely in Ukrainian.\n\n"
    ...
    try:
        ...
        data = resp.json()
        # можливі формати відповіді HuggingFace
        ...
        return f"⚠️ Помилка запиту до моделі: {e}"
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ai:open":
        ...
        text = (
            "🧠 *AI-помічник (універсальний режим)*\n"
            "Запиши запитання або тему — отримаєш відповідь українською.\n"
            "Підтримує поради, пояснення, ідеї. Не замінює фінконсультанта.\n\n"
            "Надішли повідомлення нижче 👇"
        )
        await q.edit_message_text(text, parse_mode="Markdown", reply_markup=ai_menu_ikb())
        return AI_CHAT
